...
public class EmailService {
    ...
    /**
     * Sends a password reset email with a one-hour recovery link.
     * The copy is explicit about expiry and request origin so users can quickly spot suspicious activity.
     *
    ...
    public void sendPasswordResetEmail(String toEmail, String firstName, String resetUrl) {
        try {
            ...
            helper.setFrom(from, senderName);
            helper.setTo(toEmail);
            helper.setSubject("Reset Your Bazaar Password");
    ...
    private String buildVerificationEmailHtml(String firstName, String verificationUrl) {
        return "<!DOCTYPE html>" +
                ...
                ".email-container { max-width: 600px; margin: 20px auto; background-color: #ffffff; border-radius: 8px; box-shadow: 0 2px 8px rgba(0,0,0,0.1); overflow: hidden; }" +
                ".header { background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; padding: 30px 20px; text-align: center; }" +
                ".header h1 { margin: 0; font-size: 28px; font-weight: 700; }" +
                ".content { padding: 40px 30px; line-height: 1.6; color: #333333; }" +
                ".greeting { font-size: 18px; margin-bottom: 20px; font-weight: 600; }" +
                ".message { font-size: 14px; margin-bottom: 30px; color: #555555; }" +
                ".cta-button { display: inline-block; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white !important; padding: 14px 32px; text-decoration: none; border-radius: 6px; font-weight: 600; font-size: 16px; margin: 20px 0; }" +
                ".fallback-link { margin-top: 30px; padding-top: 20px; border-top: 1px solid #e5e5e5; font-size: 12px; color: #999999; }" +
                ".fallback-link a { color: #667eea; text-decoration: underline; }" +
                ".footer { background-color: #f9f9f9; padding: 20px 30px; text-align: center; font-size: 12px; color: #888888; border-top: 1px solid #e5e5e5; }" +
                ".security-note { background-color: #e8f4f8; padding: 12px 15px; border-left: 4px solid #0088cc; margin-top: 20px; font-size: 13px; color: #0056b3; }" +
                "</style>" +
                "</head>" +
                "<body>" +
                ...
                "<div class='message'>" +
                "Thank you for signing up with Bazaar! To complete your registration " +
                "and unlock full access to our platform, please verify your email " +
     ...
     * This mirrors the verification email structure so the brand stays consistent in every recovery flow.
     */
    private String buildPasswordResetEmailHtml(String firstName, String resetUrl) {
        return "<!DOCTYPE html>" +
                ...
                ".fallback-link a { color: #667eea; text-decoration: underline; }" +
                ".footer { background-color: #f9f9f9; padding: 20px 30px; text-align: center; font-size: 12px; color: #888888; border-top: 1px solid #e5e5e5; }" +
                ".security-note { background-color: #fff4e5; padding: 12px 15px; border-left: 4px solid #f5a623; margin-top: 20px; font-size: 13px; color: #8a5a00; }" +
                "</style>" +
                "</head>" +
                "<body>" +
                "<div class='email-container'>" +
                ...
                "<div class='greeting'>Hi " + firstName + ",</div>" +
                "<div class='message'>" +
                "We received a request to reset your Bazaar password. If this was you, use the button below to create a new password and regain access to your account." +
                "</div>" +
